...
that is the `opencv@4` formula. OCR runs in-process against libtesseract and
Leptonica, so both sets of headers are a build dependency too: the brew and
Arch `tesseract` packages pull them in, Debian/Ubuntu needs `libtesseract-dev`
+ `libleptonica-dev` and Fedora `tesseract-devel` + `leptonica-devel` (the
script installs these). On Arch, whose official `opencv` package is
