...
Timing: every step has two knobs. delay (milliseconds, default 1000; negative means no delay at all) is slept BEFORE the step acts — it paces the flow and gives the previous action's screen change time to settle. Set delay -1 on the FIRST step of a fresh batch when nothing preceded it (run_route does this on its own for the route's first queued step). timeout (seconds, default 3) is how long the server keeps re-locating the target on live frames before failing — retries run back to back, and the step proceeds the moment the target shows up. For a target that appears late (app launch, a navigation tap, network loading) raise timeout (10-15) rather than delay; use a bigger delay only when the target is visible early but not yet safe to touch (mid-animation). Keep probe/visibility checks at the short default timeout so a negative answer comes back fast. Session startup never eats the step timeout — the first video frame gets its own grace period.
...
type stepInput struct {
	...
	Delay     int             `json:"delay,omitempty" jsonschema:"milliseconds slept BEFORE the step acts, default 1000; -1 means no delay — use it on the first step of a fresh batch; raise it only when the target is visible but mid-animation"`
}
