...
Timeouts: every step accepts a timeout in seconds (default 3) — the server retries locating the target about twice per second until it expires. The default suits elements already on screen; set 10-15 on steps whose target appears after an app launch, a navigation tap or network loading, and keep probe/visibility checks at the short default so a negative answer comes back fast. Session startup never eats the step timeout — the first video frame gets its own grace period.
